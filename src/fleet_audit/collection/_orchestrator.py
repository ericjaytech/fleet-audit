"""Collector execution orchestration and snapshot assembly.

This module runs named collectors, parses their output and assembles the
versioned JSON snapshot document.  It is the single entry-point for live
collection; the ``collection`` package re-exports only the public API.
"""
...
from fleet_audit.collection._parsing import ParsingError
from fleet_audit.collection.hardware_parser import parse_hardware
from fleet_audit.collection.network_parser import (
    NetworkWarning,
    parse_network,
)
from fleet_audit.collection.os_parser import parse_platform
...
_ANY_WARNINGS = StorageWarning | NetworkWarning | SoftwareWarning


def _parse_platform_capture(workspace: Path) -> tuple[dict[str, Any], tuple[()]]:
    return parse_platform(workspace), ()


def _parse_hardware_capture(workspace: Path) -> tuple[dict[str, Any], tuple[()]]:
    return parse_hardware(workspace), ()


def _parse_storage_capture(workspace: Path) -> tuple[dict[str, Any], tuple[_ANY_WARNINGS, ...]]:
    result = parse_storage(workspace)
    return result.storage, result.warnings


def _parse_network_capture(workspace: Path) -> tuple[dict[str, Any], tuple[_ANY_WARNINGS, ...]]:
    result = parse_network(workspace)
    return result.network, result.warnings


def _parse_software_capture(workspace: Path) -> tuple[dict[str, Any], tuple[_ANY_WARNINGS, ...]]:
    result = parse_software(workspace)
    return result.software, result.warnings


_PARSER_REGISTRY: dict[str, Callable[[Path], tuple[dict[str, Any], tuple[Any, ...]]]] = {
    "platform": _parse_platform_capture,
    "hardware": _parse_hardware_capture,
    "storage": _parse_storage_capture,
    "network": _parse_network_capture,
    "software": _parse_software_capture,
}
...
def collect_snapshot(
    *,
    ...
    """Run every configured collector and assemble a validated snapshot.

    Parameters
    ----------
    label:
        Non-sensitive host label stored in the snapshot.
    collector_paths:
        Override mapping of collector name to script path.  When ``None`` the
        packaged shell collectors are used.
    timeout_seconds:
        Per-collector execution timeout in seconds.
    workspace_parent:
        Parent directory for the ephemeral collection workspace.  When
        ``None`` the system temporary directory is used.

    Returns
    -------
    dict
        A dictionary conforming to the Fleet Audit snapshot schema (v1).
    """
...
def _parse_collector_output(
    ...
    parser = _PARSER_REGISTRY.get(name)
    try:
        if parser is None:
            raise CollectionError(f"unknown collector: {name}")
        domain, parser_warnings = parser(workspace)
        return domain, result, None, parser_warnings
    except (ParsingError, CollectionError) as error:
        return (
...
def _unavailable_domain(name: str) -> dict[str, Any]:
    ...
    return mapping.get(name, {"status": "unavailable"})
